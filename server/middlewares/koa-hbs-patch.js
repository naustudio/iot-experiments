...
Hbs.prototype.findTemplatePath = function (tpl) {
	var paths = (typeof this.viewPath === 'string') ? [this.viewPath] : this.viewPath;
	var viewPath = '';
	for (var i = 0; i < paths.length; i++) {
		viewPath = paths[i];
		viewPath = path.join(viewPath, tpl + this.extname);
		viewPath = fs.realpathSync(viewPath);
		if (viewPath) {
			break;
		}
	}
...
Hbs.prototype.createRenderer = function() {
	...
	return function *(tpl, locals) {
		var tplPath = hbs.findTemplatePath(tpl),//path.join(hbs.viewPath, tpl + hbs.extname),
