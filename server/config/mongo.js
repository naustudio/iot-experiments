...
comongo.connect = function *() {
	...
	var result = yield db.authenticate(config.mongo.dbUser, config.mongo.password);
	if (result) {
		console.log('DB authentication sucessful');
	}
	// export default collections
	comongo.counters = yield db.collection('counters');
	comongo.users = yield db.collection('users');
	comongo.motion = yield db.collection('motion');
};
